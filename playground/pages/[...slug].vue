...
import { useCraftCurrentSite, useCraftUri, useCraftEntry, useCraftQuery } from '#imports'
...
const { data: craftEntry2 } = useCraftEntry<BaseResT>().uri(uri.value).site(currentSite.value.handle).one()
const { data: craftAsset1 } = useCraftAsset().volume('images').one()
const { data: craftAsset2 } = useCraftAsset<BaseResT>().volume('images').one()
const { data: craftEntryQuery1 } = useCraftQuery('entries').uri(uri.value).site(currentSite.value.handle).one()
const { data: craftEntryQuery2 } = useCraftQuery<BaseResT, 'entries'>('entries').uri(uri.value).site(currentSite.value.handle).one()
const { data: craftAssetQuery1 } = useCraftAsset().volume('images').one()
const { data: craftAssetQuery2 } = useCraftQuery<BaseResT, 'assets'>('assets').volume('images').one()

console.log(craftEntry1.value, craftEntry2.value, craftAsset1.value, craftAsset2.value, craftEntryQuery1.value, craftEntryQuery2.value, craftAssetQuery1.value, craftAssetQuery2.value)
